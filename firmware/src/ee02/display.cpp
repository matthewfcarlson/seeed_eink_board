...
void Spectra6Display::clear(uint8_t color) {
    fillColor(color);
}
...
// 'A'-'Z' (everything else renders as a blank cell) - enough for status screens
// like the config-mode banner without pulling in a full graphics library.
// Indexed by (char - 0x20); each glyph is 7 rows of 5 bits (bit4 = leftmost pixel).
...
void Spectra6Display::setPixel(uint16_t x, uint16_t y, uint8_t color) {
    if (buffer_ == nullptr || x >= DISPLAY_WIDTH || y >= DISPLAY_HEIGHT) return;
...
void Spectra6Display::drawChar(uint16_t x, uint16_t y, char c, uint8_t color, uint8_t scale) {
    if (c < 0x20 || c > 0x5A) c = ' ';
...
void Spectra6Display::drawString(uint16_t x, uint16_t y, const String& text, uint8_t color, uint8_t scale) {
