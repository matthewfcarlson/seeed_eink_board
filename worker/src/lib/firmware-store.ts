 ...
 */
export async function putFirmwareBinary(env: Env, board: string, version: string, bytes: Uint8Array): Promise<void> {
  await env.KV.put(kvKeys.firmwareBin(board, version), bytes);
}

export async function getFirmwareBinary(env: Env, board: string, version: string): Promise<ArrayBuffer | null> {
  return env.KV.get(kvKeys.firmwareBin(board, version), "arrayBuffer");
}
